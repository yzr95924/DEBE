...
LeveldbDatabase::~LeveldbDatabase() {
    ...
    // clean the DB
    leveldb::Options tmpOption;
    leveldb::DestroyDB(dbName_, tmpOption); // delete this db
}
...
/**
 * @brief Get the Value Uint32 object
 * 
 * @param valueList the value list
 */
void LeveldbDatabase::GetValueUint32(vector<uint32_t>& valueList) {
    leveldb::DB* db = levelDBObj_;
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ...
        uint32_t tmpValue = 0;
        memcpy(&tmpValue, &valueStr[0], sizeof(uint32_t));
